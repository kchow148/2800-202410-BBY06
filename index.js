...
/**
 * Deletes expenses Older than 10 months old from database.
...
app.get('/home', async (req, res) => {
    ...
    //checks if there are budgets made
    if (result[0].categories === undefined) {
        res.render("home", { exist: false });
    }
    //------------------------------
    //adds up total expenses for budgets 
    else {
...
app.post('/settingBudget', async (req, res) => {
    ...
    //checks if budgetname is already made
    if (budgetname === "other") {
...
app.get('/addExpenses', async (req, res) => {
    ...
    const result = await userCollection.find({ loginID: loginID }).project({ categories: 1 }).toArray();
    console.log(result[0].categories);
    //checks if budgets are made to decide the dropdown of form
...
app.post('/addingExpenses', async (req, res) => {
    ...
    //checks if expense is default selection if it is save only in general expense collection
    if (category != "other") {
    ...
    //save to general expense collection
...
app.get('/budgets', async (req, res) => {
    ...
    const currentDate = new Date();
    //check month to show budgets of 
    if (month === undefined) {
    ...
    const result = await userCollection.find({ loginID: loginID }).project({ categories: 1 }).toArray();
    //console.log(result);
    //checks if budgets are made
    if (result[0].categories === undefined) {
        res.render("budgets", { exist: false });
    }
    //--------------------------------
    //shows expenses for each budget of the month
    else {
        budgets = result[0].categories
        let i = 0;
        let expenses = [];
...
app.get('/expenses', async (req, res) => {
    ...
    //checks month to show expenses for
    if (month === undefined) {
        currentMonth = currentDate.getMonth();
    } else {
        if (parseInt(month) === -1) {
